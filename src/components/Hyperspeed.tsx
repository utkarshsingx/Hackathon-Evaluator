...
class CarLights {
  ...
  init() {
    ...
    const distortionObj = typeof this.options.distortion === "object" ? this.options.distortion : null;
  ...
  update(time: number) {
    const u = this.mesh.material.uniforms as Record<string, { value: number }>;
    if (u.uTime) {
      u.uTime.value = time;
    }
  }
...
class LightsSticks {
  ...
  init() {
    ...
    const sticksColor = options.colors.sticks;
    let colorArray: THREE.Color[];
    if (Array.isArray(sticksColor)) {
      colorArray = sticksColor.map((c) => new THREE.Color(c));
    } else {
      colorArray = [new THREE.Color(sticksColor)];
    }
    ...
    const distortionObj = typeof options.distortion === "object" ? options.distortion : null;
    const material = new THREE.ShaderMaterial({
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // Append canvas to DOM first so WebGL context is ready for EffectComposer
  ...
  init() {
    ...
    // Defer EffectComposer setup to next frame so WebGL context is fully ready
    // (getContextAttributes() can return null if canvas isn't painted yet)
  ...
  onMouseDown(_ev: MouseEvent) {
    if (this.options.onSpeedUp) this.options.onSpeedUp(_ev);
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }

  onMouseUp(_ev: MouseEvent) {
    if (this.options.onSlowDown) this.options.onSlowDown(_ev);
